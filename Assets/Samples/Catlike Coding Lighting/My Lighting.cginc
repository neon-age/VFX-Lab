...
void ApplyParallax (inout Interpolators i, out float2 uvOffset) {
	uvOffset = 0;
	#if defined(_PARALLAX_MAP)
		...
		#endif
		uvOffset = PARALLAX_FUNCTION(i.uv.xy, i.tangentViewDir.xy);
...
FragmentOutput MyFragmentProgram (Interpolators i) {
	...
	float2 uvOffset = 0;
	ApplyParallax(i, uvOffset);
	float4 pos_clip = mul(UNITY_MATRIX_VP, float4(i.worldPos.xyz, 1.0));
	output.outDepth = pos_clip.z / pos_clip.w;
	//output.outDepth = (pos_clip.z - (abs(uvOffset.x) + abs(uvOffset.y)) ) / pos_clip.w;
	//    if(abs(uvOffset.x) > 0.2 || abs(uvOffset.y) > 0.2) 
		//		discard;
